  ...
  return (
    ...
    >
      ...
      >
        ...
        {auditTab ? (
          ...
          >
            ...
            <DealClosePanel threadId={threadId} thread={activeThread} placement="composer" />
            {!auditTab && activeThread && showContextTriggerBar ? (
              <div
                className={`hidden shrink-0 border-t border-border/50 bg-[var(--df-bg-elevated)]/90 py-1.5 md:block ${crmSide ? "xl:hidden" : ""} ${INBOX_CHAT_GUTTER_X}`}
              >
                <Button variant="secondary"
                  type="button"
                  onClick={() => setCrmDrawerOpen(true)}
                  className="flex w-full items-center justify-center gap-1.5 rounded-md border df-border-brand bg-[var(--df-bg-app)] px-2 py-1.5 text-[11px] font-semibold text-[var(--df-text-secondary)] shadow-sm transition hover:border-[var(--df-brand-500)]/40 hover:bg-[var(--df-brand-50)]"
                >
                  <svg className="h-3.5 w-3.5 shrink-0 text-[var(--df-text-muted)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden>
      ...
      {!auditTab && activeThread && crmDrawerOpen && showContextTriggerBar ? (
        <div
          className={`fixed inset-0 z-[45] md:block ${focusContextOnDemand ? "" : "xl:hidden"}`}
          role="presentation"
        >
