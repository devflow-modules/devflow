...
export function AuthScreenShell({ eyebrow, title, description, children, footer }: Props) {
  return (
    <main className="flex h-full min-h-0 flex-col overflow-y-auto bg-slate-50/90 p-6">
      <div className="mx-auto flex w-full max-w-md flex-1 flex-col justify-center py-6">
        <div className="w-full rounded-2xl border border-slate-200/90 bg-white p-8 shadow-sm">
          {eyebrow ? (
          ...
          <h1
            className={`text-center text-2xl font-semibold tracking-tight text-slate-900 ${eyebrow ? "mt-2" : ""}`}
          >
            {title}
          </h1>
          {description ? (
