...
export function ChannelVerificationCard({ channelId, onTimelineRefresh }: Props) {
  ...
  if (loading) {
    return (
      <section className="mb-6 rounded-xl border border-slate-200 bg-slate-50/80 p-4" aria-busy>
  ...
  return (
    <section
      className="mb-6 rounded-xl border border-amber-200/90 bg-amber-50/40 p-4"
      data-testid="channel-verification-card"
    >
      {toastAnchor}
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <h3 className="df-label text-amber-950">Verificação Meta Business</h3>
          <p className="mt-0.5 text-xs text-slate-600">
            Estado operacional interno — não substitui o fluxo oficial no Meta Suite.
          </p>
        </div>
        <span className="shrink-0 rounded-full bg-white px-2.5 py-1 text-xs font-semibold text-slate-800 ring-1 ring-slate-200">
          {statusLabel(status)}
        </span>
      </div>
      ...
      <ul className="mt-4 space-y-2">
        {checklist.items.map((item) => (
          <li key={item.id} className="flex items-start gap-2 rounded-lg border border-slate-100 bg-white/90 px-2 py-2">
            <input
              type="checkbox"
