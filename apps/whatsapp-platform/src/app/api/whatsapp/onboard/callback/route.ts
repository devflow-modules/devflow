 ...
 * Valida state = tenantId e que o usuário tem acesso ao tenant.
 */
...
export async function POST(request: NextRequest) {
  ...
  if (state !== userTenantId) {
    console.warn(`[WHATSAPP][onboard/callback] state mismatch: state=${state} userTenant=${userTenantId}`);
    return NextResponse.json(
  ...
  try {
    ...
    for (const pn of phoneNumbers) {
      ...
      try {
        encryptedFields = buildEncryptedAccessTokenFields(pn.accessToken);
      } catch (e) {
