...
/** Rótulos de linha de uso adicional na UI (texto neutro para o cliente). */
export const STRIPE_USAGE_LINE_LABELS = {
  extraConversations: "Conversas adicionais (fora do incluído)",
  extraAi: "Uso adicional de IA (fora do incluído)",
} as const;
...
export function formatIncludedUsageSentence(plan: PlanKey): string {
  ...
  if (parts.length === 0) return "Inclui uso conforme o contrato.";
...
export const USAGE_AFTER_INCLUDED_EXPLAINER = isWhiteLabelMode()
  ...
  : "Ao atingir o volume incluído no contrato, o uso pode continuar e o adicional é registado no fim do período mensal, conforme acordado.";
...
export const USAGE_ANTI_SURPRISE_LINE = isWhiteLabelMode()
  ...
  : "Não interrompemos o atendimento por causa dos limites incluídos: quando o volume vai além do pacote contratado, regista-se o uso adicional e o extrato do período mostra isso com transparência — sem surpresas.";
    ...
    if (options?.isFreePlan) {
      ...
      const usedPart =
        typeof used === "number" && used >= 0
          ...
          : "";
      return `Ambiente de avaliação guiada: até ${messagesLimit.toLocaleString("pt-BR")} conversas incluídas por mês.${usedPart} Ao atingir o limite, a operação completa é ativada com a implantação — Contrato e uso.`;
    }
    return `O seu pacote inclui até ${messagesLimit.toLocaleString("pt-BR")} conversas por mês. Se precisar de mais, o uso adicional está disponível — veja detalhes em Contrato e uso.`;
  }
  return "Veja limites incluídos e uso do período em Contrato e uso.";
}
...
export function contextualAiUsageHint(
  ...
  if (aiLimit != null && aiLimit > 0) {
    ...
    return `Inclui até ${aiLimit.toLocaleString("pt-BR")} interações de IA por mês no pacote contratado. Além disso, pode expandir com uso adicional — sem interrupção do serviço.`;
  }
  return "Consulte o consumo de IA e os limites em Contrato e uso.";
}
...
export function freePlanUsageExplainerLines(plan: PlanKey): { title: string; bullets: string[] } {
  ...
  bullets.push(
    "Ao atingir o incluído, é preciso avançar para a operação completa — fale connosco ou use Contrato e uso."
  );
