  ...
  return (
    <section className="il-card p-5">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <button type="button" className={btn} onClick={() => void runCopy(buildChatGptSessionExport({ session, problem }))}>
          Copy session for ChatGPT
        </button>
        <button type="button" className={btn} onClick={() => void runCopy(buildFailedTestsExport({ session, problemTitle: title }))}>
          Copy failed tests
        </button>
        <button type="button" className={btn} onClick={() => void runCopy(session.code)}>
          Copy code only
        </button>
        <button type="button" className={btn} onClick={() => void runCopy(buildExplanationTemplate())}>
