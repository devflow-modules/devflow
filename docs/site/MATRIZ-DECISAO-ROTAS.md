...
| `/career-automation/execute` (POST, host ApplyFlow) | `apps/applyflow` | Só app | ok | **manter** — server-side approved automation execution boundary; client-safe `CareerAutomationExecutionResult` only; feature-flagged (`CAREER_AUTOMATION_ENABLED` default off, `CAREER_AUTOMATION_PROVIDER=mock\|openclaw`, `OPENCLAW_ENABLED` default off); explicit, request-scoped approval; **single execution** (one request → one tool → one execution); `retryCount` max 0 (no retry; new attempt needs new approval); reconstructs execution plan → proposal → tool definition → capability → approval → policy → tool invocation server-side; server-authoritative kind→tool allowlist (OpenClaw never selects kind/tool/capability/args/risk/approval/retry/schedule/next action); real OpenClaw HTTP transport in app layer (`POST <baseUrl>/v1/executions`, Bearer server-side) with strict response validation; **no schedule / no background / no persistence / no persistent memory / no arbitrary tool selection**; secrets and base URL server-side only; mandatory human review; `executedExternally:true` only means the OpenClaw adapter was called; `GET` → 405 |
| `/career-automation/health` (GET, host ApplyFlow) | `apps/applyflow` | Só app | ok | **manter** — internal approved-automation health/status; client-safe JSON only (`enabled`, `provider`, `configured`, `reachable`, `timeoutMs`); no secrets/base URL/internal IDs; does not contact OpenClaw per request (`reachable` null unless explicit `?probe=true` controlled lookup); `POST` → 405 |
