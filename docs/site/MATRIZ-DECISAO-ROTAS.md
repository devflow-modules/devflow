...
| `/provider-runtime/nango/connect` (POST, host ApplyFlow) | `apps/applyflow` | Só app | ok | **manter** — launcher server-side Nango Connect session; POST only (GET → 405); client-safe JSON + short-lived connect session token when allowed; anonymous `af_nango_caller` cookie is a browser session, not a user login; same-origin Origin required when runtime is on; requires feature flags + explicit consent; no Gmail/Calendar import; no sync job; no raw payload persistence; no OAuth token exposure; no CareerBundle mutation |
...
| `/career-chat/librechat` (POST, host ApplyFlow) | `apps/applyflow` | Só app | ok | **manter** — server-side LibreChat-compatible chat adapter; client-safe `CareerChatResponse` only; deterministic intent from `action`; orchestrator server-side; tool proposals without execution; explicit consent; feature flag `LIBRECHAT_ADAPTER_ENABLED`; optional real transport `LIBRECHAT_TRANSPORT_ENABLED` (default off); PR #123 adds specialist actions `analyze_resume`/`analyze_ats_compatibility`/`plan_career_strategy` over the same boundary; no LLM/provider calls; no conversation persistence; `GET` → 405 |
| `/career-chat/librechat/health` (GET, host ApplyFlow) | `apps/applyflow` | Só app | ok | **manter** — internal LibreChat transport health/status; client-safe JSON only; no secrets; `POST` → 405 |
| `/career-llm/generate` (POST, host ApplyFlow) | `apps/applyflow` | Só app | ok | **manter** — server-side controlled LLM generation boundary; client-safe `CareerLlmResult` only; feature-flagged (`CAREER_LLM_ENABLED` default off, `CAREER_LLM_PROVIDER=mock\|openai`); explicit consent; structured output validated against schema + limits; reconstructs chat → orchestration → task → policy → provider server-side; OpenAI provider uses Responses API with strict Structured Outputs (`store:false`, `stream:false`, no tools); server-owned model/timeout/retries; no silent openai→mock fallback; LLM never selects intent/agent/task/provider/tools and never executes tools; **no tool execution** (no `/career-tools/invoke`); PR #123 adds optional explanation tasks for the specialist agents (LLM only explains/organizes deterministic results, never computes scores); secrets server-side only; no persistence; mandatory human review; `GET` → 405 |
| `/career-llm/health` (GET, host ApplyFlow) | `apps/applyflow` | Só app | ok | **manter** — internal controlled LLM health/status; client-safe JSON only (`enabled`, `provider`, `configured`, `modelAlias`, `reachable`); no secrets/raw model id/internal URL; does not call the provider API per request (`reachable` null unless explicit `?probe=true` controlled lookup); `POST` → 405 |
...
| `/career-system/health` (GET, host ApplyFlow) | `apps/applyflow` | Só app | ok | **manter** — aggregated client-safe Career Suite health across existing boundaries; no external probe by default (`?probe=true` bounded); `503` when unhealthy; no token/tool/provider payload/persistence; `POST` → 405 |
| `/career-system/livez` (GET, host ApplyFlow) | `apps/applyflow` | Só app | ok | **manter** — liveness; process responds only; no OpenAI/LibreChat/Nango/Gmail/Calendar/tools/DB; `POST` → 405 |
