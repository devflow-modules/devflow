...
- **2026-07-30** — PR #182 tocou `apps/whatsapp-platform/src/app/settings/ai/page.tsx` apenas para o submit «Salvar alterações» no `PageHeader`: de `Button` (com `trackedOnClick`) para `<button type="submit">` + `buttonClassName("primary")`, alinhado a `settings/billing`. Motivo: em Turbopack/RSC local, `Button` no Server Component quebrava a página (`Event handlers cannot be passed to Client Component props`). Sem mudança de path, ownership, navegação, auth, contrato HTTP ou semântica do formulário. O F0 do mesmo PR (CTA «Testar resposta») não altera rotas.
