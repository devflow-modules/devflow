...
- **2026-07-30** — PR #182 tocou `apps/whatsapp-platform/src/app/settings/ai/page.tsx` para extrair o submit «Salvar alterações» do `PageHeader` para o Client Component `AiSettingsSaveHeaderButton` (`Button` + `form="wf-ai-settings"`). Motivo: montar `Button` (com `trackedOnClick`) directamente no Server Component quebrava a página sob Turbopack/RSC (`Event handlers cannot be passed to Client Component props`); `check:buttons` continua a exigir o primitivo `Button`. Sem mudança de path, ownership, navegação, auth, contrato HTTP ou semântica do formulário. O F0 do mesmo PR (CTA «Testar resposta») não altera rotas.
...
- **2026-07-31** — PR #209 (SEC-1a / `security/whatsapp-token-encryption-sec1a`): **nenhuma rota nova**; handlers `route.ts` existentes em `apps/whatsapp-platform` (`POST/PATCH` activate channel, `PATCH /api/tenants/me`, `POST /api/whatsapp/onboard/callback`, list/detail admin channels) alterados só na **implementação interna** — dual-write de token Meta (`accessToken` + `accessTokenEncrypted`), fail-closed criptográfico, presença sem decrypt, redaction de respostas. Paths, métodos HTTP, ownership (`whatsapp-platform`), autenticação/autorização, isolamento por tenant, navegação e topologia **inalterados**; sem redirects. Registry `routing-governance.ts` não muda (sem path novo a registar).
- **2026-08-03** — SEC-1-final (`security/whatsapp-token-encryption-sec1-final`): **nenhuma rota nova**; mesmos handlers — implementação interna passa a **encrypt-only** (`accessTokenEncrypted` apenas; coluna `access_token` removida). Paths/métodos/ownership/auth/tenancy/topologia **inalterados**.
- **2026-08-03** — PR #213 (`fix/inbox-send-safe-retry`): **nenhuma rota nova**; `POST /api/inbox/conversations/[id]/send` em `apps/whatsapp-platform` altera o **contrato do body** (`clientRequestId` obrigatório) e a implementação interna (ledger `WaInboxSendRequest`, claim CAS, retry/reconcile sem segundo `sendText` Meta). Path, método HTTP, ownership (`whatsapp-platform`), autenticação, isolamento por tenant, navegação e topologia **inalterados**; sem redirects. Registry `routing-governance.ts` não muda (sem path novo a registar). Migration aditiva `wa_inbox_send_requests` ainda não aplicada em Production neste Draft.
- **2026-08-04** — PR #214 (`feat/inbox-authorship-control`): **nenhuma rota nova**; `POST /api/inbox/conversations/[id]/send` passa a rejeitar com **409** `THREAD_CLOSED` / `THREAD_ASSIGNED_TO_OTHER` (e fail-closed para status/caller desconhecido) antes de novo `sendText` Meta; replay `COMPLETED` e reconcile `META_ACCEPTED` do ledger **não** reentram no gate de autoria. UI do composer bloqueia e oferece Reabrir (`POST …/status` OPEN) / Assumir (`POST …/assign` CAS existente). Path, método HTTP, ownership, autenticação, tenancy e topologia **inalterados**; sem redirects nem migration. Registry `routing-governance.ts` não muda.
- **2026-08-04** — `feat/inbox-sending-reconcile`: **rotas novas** (ownership `whatsapp-platform`, sem cutover de portal): `POST /api/cron/inbox-send-reconcile` (Bearer `CRON_SECRET`, comparação exacta; dry-run sem mutação); `GET|POST /api/admin/inbox-send-requests` (manager+, tenant-scoped); `POST /api/admin/inbox-send-requests/[id]/acknowledge` (manager+, tenant-scoped; **não** chama Meta; status permanece `UNKNOWN_OUTCOME` — não reabre elegibilidade de retry). `POST /api/inbox/conversations/[id]/send` rejeita ledger `UNKNOWN_OUTCOME` com **409** sem Meta. Registry `routing-governance.ts` não muda (rotas internas de plataforma; sem redirect de domínio). Migration aditiva enum `UNKNOWN_OUTCOME` **ainda não aplicada em Production** neste Draft — rollout **migration-first**.
- **2026-08-04** — PR #217 (`fix/run-rules-tenant-authz`): **nenhuma rota nova**; `POST /api/automation/run-rules` em `apps/whatsapp-platform` altera só **autorização server-side** — sessão `manager`|`platform_admin` força `auth.payload.tenantId` (body.tenantId alienígena → **403**); lote all-tenants permanece exclusivo de Bearer `CRON_SECRET`/`BILLING_CRON_SECRET`. Path, método HTTP, ownership, topologia e registry **inalterados**; sem redirects nem migration.

- **2026-09-16** — Career OS v2 no host ApplyFlow (`apps/applyflow` only; **not** portal `src/app`). New UI: `/dashboard/analytics` and `/dashboard/jobs/[id]` — local-first pages; data from browser storage after hydration; **no server login**; no equivalent routes on the portal; no redirects. New API: `POST /provider-runtime/nango/inbound-responses` — Gmail closed-loop inbound metadata scan; blocked-by-default (runtime flags + explicit consent + server Nango verification); account scope required when more than one mailbox; client-safe JSON; no Gmail mutation; no raw mail persistence; `GET` → 405. `layout.tsx` only sets `metadataBase` from `NEXT_PUBLIC_APPLYFLOW_URL` / `VERCEL_URL` (not a new path). Registry `routing-governance.ts` unchanged (portal paths only). Host: Vercel project `devflow-applyflow` (root directory `apps/applyflow`); Production Git branch is `production` (not `main`); public production alias currently `https://devflow-applyflow.vercel.app`.
- **2026-09-17** — `POST /provider-runtime/nango/connect` in `apps/applyflow` replaces GET for Connect Session creation. GET returns 405 and does not mint `af_nango_caller` or call Nango. POST requires a same-origin `Origin` header matching `request.nextUrl.origin`, a server-validated anonymous caller cookie (or mints one), and never treats `explicitConsent` as login. Connection tags are `applyflow-{provider}-{hash(callerNonce)}`; the retired shared ids `applyflow-*-runtime-boundary` are not queried. `POST` connection-status, derived-preview, inbound-responses and disconnect use the same caller + origin guards. Cookie is not a user session. Host: ApplyFlow only; flags remain default-off.
- **2026-09-22** — PR #226 (`fix/financeiro-household-refs`): **nenhuma rota nova**. Handlers existentes em `apps/financeiro` (`/api/expenses`, `/api/incomes`, `/api/rules`, `/api/budgets`, `/api/payment-days`, `/api/month-snapshots`, `/api/recurrence`) passam a recusar FK de outra household (`HOUSEHOLD_REF_NOT_FOUND`) e a exigir OWNER no fechamento/recorrência. Paths, métodos, ownership e registry **inalterados**.
- **2026-09-22** — PR #230 (`fix/whatsapp-phone-number-patch-role`): **nenhuma rota nova**. `PATCH /api/whatsapp/phone-numbers/:id` em `apps/whatsapp-platform` passa a exigir `requireRole(ROLES_MANAGER_PLUS)`, alinhado ao DELETE. Path, método, ownership, tenancy e registry **inalterados**.
