...
- **2026-06-18** — PR #122 integra o adapter externo OpenClaw de execução única em `apps/applyflow` (sem novo endpoint de execução): `POST /career-automation/execute` permanece o único boundary; o `createOpenClawCareerAutomationAdapter` faz transporte HTTP real (`POST <baseUrl>/v1/executions`, `fetch` server-side + `AbortController`) enquanto o core `career-automation` continua sem rede/SDK/timers/fs. Flags server-side default-off (`CAREER_AUTOMATION_ENABLED`, `CAREER_AUTOMATION_PROVIDER=mock|openclaw`, `OPENCLAW_ENABLED`, `OPENCLAW_BASE_URL`, `OPENCLAW_API_KEY`, `OPENCLAW_TIMEOUT_MS=10000`); `mock` padrão; **sem fallback silencioso** openclaw→mock; secrets e base URL nunca no cliente. Auth `Authorization: Bearer <OPENCLAW_API_KEY>` server-side; **single execution** (uma request → uma tool → uma execução); `retryCount` máximo 0 (sem retry; nova tentativa exige nova approval); **sem schedule/background/queue/cron/callback/webhook/persistência/memória persistente**; OpenClaw nunca escolhe kind/tool/capability/arguments/risk/approval/retry/schedule/next action (mapeamento `kind→tool` server-authoritative). Envelope mínimo (`requestId`, `proposalId`, `automationKind`, `approvedTool`, `approvedCapability`, `sanitizedInput`, `executionMode: single_execution`, `reviewRequired: true`); validação estrita da resposta (match de `proposalId`/`automationKind`/`toolName`, client-safe, rejeita `nextAction`/`toolCalls`/`commands`/`filesystem`/`schedule`/`background`/`callback`/`webhook`/`memory`/`session`); erros client-safe (`openclaw_disabled`, `openclaw_not_configured`, `openclaw_auth_failed`, `openclaw_timeout`, `openclaw_unreachable`, `openclaw_request_failed`, `openclaw_response_invalid`, `openclaw_proposal_mismatch`, `openclaw_tool_mismatch`, `openclaw_unsafe_response`); observabilidade sem secrets (`provider`, `durationMs`, `validationStatus`, `externalProviderCalled`, `retryCount`, `automationKind`, `toolName`). Adiciona `GET /career-automation/health` (`career-automation/health/route.ts`): status client-safe (`enabled`, `provider`, `configured`, `reachable`, `timeoutMs`); sem secrets/base URL/IDs internos; `reachable` null sem probe (probe explícito `?probe=true`); `POST` → 405. `executedExternally: true` significa apenas que o adapter OpenClaw foi chamado; revisão humana obrigatória.
