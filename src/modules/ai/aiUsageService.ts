 ...
 * Cópia em src/ para resolução no build Vercel (Turbopack não resolve apps/whatsapp-platform).
 * Manter sincronizado com apps/whatsapp-platform/src/modules/ai/aiUsageService.ts
 */

import { prisma } from "@/lib/prisma";
import { AiUsageLogType } from "@/generated/prisma-whatsapp";

// Inline para evitar dependência de apps/whatsapp-platform
function estimateCostFromTotal(totalTokens: number): number {
  const inputEst = Math.floor(totalTokens * 0.7);
  const outputEst = totalTokens - inputEst;
  const INPUT = 0.15 / 1_000_000;
  const OUTPUT = 0.6 / 1_000_000;
  return inputEst * INPUT + outputEst * OUTPUT;
}
