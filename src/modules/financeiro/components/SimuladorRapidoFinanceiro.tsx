...
export function SimuladorRapidoFinanceiro() {
  ...
  const simulatorTracked = useRef(false);

  const reportSimulatorUsed = () => {
    if (simulatorTracked.current) return;
    simulatorTracked.current = true;
    trackSimulatorUsed();
  };
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        <label className="space-y-1">
          ...
          <input
            ...
            onChange={(e) => {
              reportSimulatorUsed();
              setReceita(e.target.value.replace(/[^\d.,\s]/g, ""));
            }}
            placeholder="ex.: 6500"
        ...
        <label className="space-y-1">
          ...
          <input
            ...
            onChange={(e) => {
              reportSimulatorUsed();
              setVariaveis(e.target.value.replace(/[^\d.,\s]/g, ""));
            }}
            placeholder="ex.: 1750"
            className={inputClass}
