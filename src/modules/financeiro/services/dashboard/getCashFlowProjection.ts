...
import {
  ...
  startOfMonth,
} from "@/modules/financeiro/lib/cashFlowProjection";
import type { Scenario } from "@/modules/financeiro/lib/cashFlowProjection";
...
export type CashFlowProjectionSeriesItem = {
  date: string;
...
export type CashFlowProjectionResult = {
  ...
  series: CashFlowProjectionSeriesItem[];
  meta: {
    scenario: string;
    avgMonths: number;
...
export async function getCashFlowProjection(
  ...
  const from = startOfDayUTC(params.from ?? new Date());
  const horizonMonths = params.horizonMonths ?? null;
  const horizonDays =
    params.horizonDays ?? (horizonMonths ? horizonMonths * 30 : 30);
  const to = params.to
    ? startOfDayUTC(params.to)
    : startOfDayUTC(addDaysUTC(from, horizonDays));
  ...
  const [paymentDays, sources, expensesInRange, incomesForAverage, incomesBefore, expensesBefore] =
    await Promise.all([
      prisma.paymentDay.findMany({
        where: { source: { householdId: params.householdId } },
        select: { sourceId: true, dayOfMonth: true },
      }),
      prisma.source.findMany({
        where: { householdId: params.householdId, isActive: true },
        select: { id: true },
      }),
      ...
      prisma.income.findMany({
        where: {
          ...
          status: "RECEIVED",
          sourceId: { not: null },
          receivedAt: { gte: avgWindowStart, lt: from },
        },
      ...
      prisma.expense.findMany({
        where: { householdId: params.householdId, dueDate: { lt: from } },
        select: { amount: true, status: true },
      }),
    ]);

  const activeSourceIds = new Set<string>(sources.map((s) => s.id));

  const { estimateBySource: incomeEstimateBySource } = computeMonthlyAverageBySource({
    incomes: incomesForAverage.map((i) => ({
      sourceId: i.sourceId,
  ...
  const totalExpensesBefore = expensesBefore.reduce(
    (acc, e) => (e.status === "PAID" ? acc : acc + Number(e.amount ?? 0)),
    0
  );
  ...
  const series: CashFlowProjectionSeriesItem[] = [];
