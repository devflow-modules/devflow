...
O app **na raiz** (`src/`) é o que tipicamente responde em **devflowlabs.com.br**: marketing, ferramentas, entradas de outros produtos e **portal** (com redirects **308** para o host canónico quando aplicável). O **runtime do produto WhatsApp** (auth, billing, webhook Meta, dashboard) está em **`apps/whatsapp-platform`**, deployável em domínio próprio; a raiz mantém landings de marketing WhatsApp e roteamento via `NEXT_PUBLIC_WHATSAPP_APP_URL` + `@devflow/whatsapp-routes`. Os projetos em **`apps/*`** podem ser deployados em hosts separados.
...
graph TD
  ...
  packages --> whatsapp_routes["packages/whatsapp-routes — cutover 308 / JWT prefixes"]
...
| `apps/whatsapp-platform` | Produto **WhatsApp Platform** (Next.js): app canónico — auth, Stripe, webhook, dashboard; DB/env próprios. |
| `apps/investigamais`, `apps/funklab`, `apps/ops` | Scaffolds para produtos futuros. |
...
| `packages/whatsapp-core` | Utilitários / núcleo WhatsApp API (compartilhado com apps). |
| `packages/whatsapp-routes` | Rotas e regras de **cutover** portal → app (`getWhatsappCutoverRedirectUrl`, prefixos JWT). |
| `packages/ai-core` | Placeholder para adaptadores LLM/IA. |
| `packages/testing-utils` | Mocks e helpers de teste. |

A raiz do repo (`devflow/`) continua como um app Next.js (site, ferramentas, portal e camada de entrada). Produtos maduros como **WhatsApp Platform** e **Financeiro** têm app dedicado em `apps/*` com deploy e domínio separáveis. Ver runbooks de cutover em `docs/architecture/` para o estado operacional em produção.
